...
int main()
{
    ...
    do // laço de repetição para ver se o usuario quer entender como funciona o jogo ou jogar imediatamente.
    {
        ...
        {
            wprintf(L"Opção não encontrada, favor redigite!");
        }
    ...
    int pergunta1[n], pergunta2[n], pergunta3[n], pergunta4[n], pergunta5[n]; // matrizes para armazenar os erros e acertos do jogadores por pergunta
    int acertosporjogador[n]; //matriz para armazenar o desempenho individual por jagador
    ...
    for (i = 0; i < n; i++) // laço de repetição para repetir as perguntas para todos os jogadores
    {
        ...
        do // laço de repetição para confirmar que o usuário digitará uma resposta correspondente ao que foi solicitado.
        {
            ...
            scanf("%d", &rr1);
            switch (rr1) //comando de multipla escolha para conferir se a resposta esta correta
            {
            case 1:
        ...
        if (acertosporjogador[i] == 50) // if encadeado para verificar a o pontuação de cada jogador
        {
    ...
    for (i = 0; i < n; i++) // laço de repetição para impressão do desenpenho individual de cada jogador
    {
        wprintf(L"\nJogador %d - %d%% de aproveitamento", i + 1, (acertosporjogador[i] / 5) * 10);
    }
    wprintf(L"\n----------- Informações por pergunta ----------");
    for (i = 0; i < n; i++) // laço para verificar quantas vezes os jogadores acertaram cada pergunta, para depois verificar qual foi o percentual de acerto de cada pergunta.
    {
